...
| **Context** | The current situation Den assembles for a turn. | `bears.context_profile`; scoped prompt/context blocks in `prompt_memory_blocks`; ACP session/workspace binding in `acp_sessions`; workspace context on `conversations` and `bear_work_plans`; context composition code in `services/den/src/core/bears/context_composition.rs`, `services/den/src/api/acp/prompt_context.rs`, and `services/den/src/api/acp/prompt_guidance.rs`; runtime environment exposure in `services/den/src/core/tools/environment.rs`. |
