...
Implementation status (2026-06): canonical memory is per-Bear SQLite `memory_records` ([ADR-0031](adr-0031-sqlite-first-canonical-store-for-bear-agent-memory-and-tasks.md)); `memory_search` is hybrid SQL/Qdrant/graph/temporal when recall is configured; `salience`, `valid_from`, `invalid_at`, and `memory_harvest_marks` exist in the store; and store-level appends can write `supersedes_memory_id` while invalidating predecessors. The remaining gap is policy: `curate`/consolidation still needs to decide when to harvest, dedupe, supersede, and promote.
