...
> **Scope amendment (see [ADR-0034](adr-0034-jobs-and-tasks-work-management.md)):** This ADR originally claimed *all* task state for SQLite. That is narrowed here. **All task state — both Den-orchestrated jobs and lightweight session-bound tasks — is Docket-canonical and lives in Den Postgres** per ADR-0034. SQLite stores no tasks. The deciding metaphor: a Bear *uses* Den's job-management platform (Docket) to organize its work, the way a person uses a project tracker. The tracker is infrastructure the Bear plugs into, not part of the Bear. SQLite remains canonical for what is genuinely Bear cognition — role-scoped and shared/promoted **memory** — because that is where the pollution-risk and portability concerns below actually bite. The boundary that this ADR protects is **memory, not tasks**. This split holds only under the execution invariant stated in ADR-0034: Den dispatches task work to the Bear's own role runtime *within the Bear's scoped memory context*, and must not execute task bodies via generic non-Bear subagents. If that invariant were broken, task state would be smuggling Bear cognition into the control plane and this amendment would no longer hold.
...
SQLite stores **no task state**. There is no bear-local task store. All tasks — session-bound and job-bound alike — are Docket-canonical in Den Postgres per the scope amendment above and ADR-0034. The boundary is a clean line: **memory is bear-canonical (SQLite); tasks and jobs are Docket-canonical (Postgres)**, with no overlap and no sync seam between them.
