...
- Run the conductor for the curate agent: fetch peer branches before each cycle, construct the curate prompt, drive the Letta API stream, execute curate's privileged tool calls (task-intent approval/rejection, skill-proposal approval, etc.), record cycle metadata.
...
- Trigger curate runs on appropriate cadence (idle detection, message-count thresholds, pending-intent/observation/result/proposal triggers, manual trigger).
...
- Failure isolation: a crashing talk harness doesn't affect pair availability; a failing curate run doesn't block user turns; a misbehaving work task doesn't compromise channel agents.
...
- Cross-channel learning transfer is eventually-consistent. A user teaching the talk agent something on Slack does not affect the pair agent's behavior in the IDE until a curate run promotes the learning to `core/`.
- Task execution is also eventually-consistent — a user requesting work via talk or pair does not see action until the next curate run approves the request. Acceptable for scheduled and background work; not appropriate for urgent operations.
...
- Task intent and skill proposal backlog: if curate runs fall behind, user-requested work and learning piles up unactioned.
