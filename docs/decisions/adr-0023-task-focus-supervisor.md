...
status: superseded
superseded_by: adr-0035-den-native-in-process-agent-runtime.md, adr-0039-trust-profiles-and-governance-modes.md
date: 2026-05-06
---

# ADR: Den Task Focus Supervisor for Letta Agents

> **Superseded (2026-06): concept retained, re-homed.** This ADR is Letta-framed ("Letta executes, Den supervises around it"); the supervisor is now a phase of the Den-native in-process loop ([ADR-0035](adr-0035-den-native-in-process-agent-runtime.md)), not an external wrapper. Two further realignments apply:
>
> - **Continuation bias is governance, not a trust profile.** This ADR ties "drive harder vs. yield earlier" to the `pair`/`work` role. That axis is now run-scoped **governance** ([ADR-0039](adr-0039-trust-profiles-and-governance-modes.md)): `interactive` yields/asks, `autonomous_continuation` drives to completion, `grace` is the transition, `frozen` stops, `observational` never nudges. Trust profile *defaults/modulates* the bias; it does not define it. A `pair` run can be in `autonomous_continuation` and should then be driven hard.
> - **The durable objective is a focused Job.** `work` normally requires a focused Docket Job; `pair` normally has none, but may designate one explicitly through Bear conversation or client command. This is intentionally not a generic `focus_target` abstraction yet.
> - **Definition of done is Docket acceptance criteria.** The "what counts as finished" contract is `bear_job_criteria` ([ADR-0034](adr-0034-jobs-and-tasks-work-management.md)), injected into dispatch as the success contract, with `command` criteria as hard completion gates — not an independently inferred goal.
> - **Concrete loop control now lives in ADR-0050.** Tool-call budgets, wall-clock budgets, repeated-tool/ko detection, checkpoint nudges, focused-Job nudges, and Docket/task-list reconciliation are specified by [ADR-0050 (Agent Loop Control, Adaptive Budgets, and Runtime Checkpoints)](adr-0050-agent-loop-control-adaptive-budgets-and-runtime-checkpoints.md) and its roadmap plan, [`AGENT_LOOP_CONTROL_IMPLEMENTATION_PLAN.md`](../roadmap/AGENT_LOOP_CONTROL_IMPLEMENTATION_PLAN.md).
>
> The ephemeral **task focus** record below is retained as a *projection* of `(governance × focused Job × acceptance-criteria state × run/task status)`, not a fourth parallel state machine. Its states map on: `active`/`verifying`/`completed` → Docket `task_status`/`run_state`; `awaiting_user`/`awaiting_approval`/`blocked` → governance posture + handoff ([ADR-0026](adr-0026-work-handoff-and-human-escalation.md)). Read the design below through this mapping.
