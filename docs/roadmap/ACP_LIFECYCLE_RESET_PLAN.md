...
> **Direction changed (2026-06).** Turn lifecycle is Den-owned and in-process; Letta is not the source of truth for replay, obligations, continuation, or cancellation. Treat Letta-state bridging here as historical. Canonical target: [Den runtime](../architecture/den-runtime.md) ([runtime plan](DEN_RUNTIME_PLAN.md)).
...
- Adapter-side lifecycle tests from this plan are now complete: mode startup race, MCP log summarization, same/different conversation overlap, explicit session cancellation, and `/status` parsing/rendering are covered. Adapter can mirror Den-provided runtime/context-budget metadata through `session_info_update._meta.bears`.
...
Status: initial implementation added and tested in adapter as `/status`; it renders Den runtime state plus adapter-local task/MCP summaries. It should be tightened later to render canonical `session_info.runtime` once Den exposes live snapshots broadly.
...
- `AcpLettaSseStream` no longer owns terminal lifecycle state via `stream_terminated`, deferred terminal events, or deferred `RoleTurnResult`s.
- Terminal readiness is gated by `AcpTurnController`; the stream emits `turn_complete` and `turn_result` only when the controller authorizes terminal emission.
- Diagnostics such as `saw_turn_complete` no longer decide normal terminal completion.
