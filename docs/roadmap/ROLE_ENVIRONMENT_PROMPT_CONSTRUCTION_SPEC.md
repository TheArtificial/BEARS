...
> **Direction changed (2026-06).** There are no "direct Letta/API role harnesses" — all stances use one Den prompt compiler + capability profile on the in-process loop. Canonical target: [Den runtime](../architecture/den-runtime.md) ([runtime plan](DEN_RUNTIME_PLAN.md)).
