...
8. **Execution surface is explicit in typed dispatch state, never inferred from task prose.** Public dispatch targets are `local` (the current attached workspace) and `sandbox` (an isolated different workspace). From `pair`, omission selects local only with exactly one attached workspace, selects sandbox with none, and fails on multiple attachments; from `chat`/UI, omission defaults to sandbox. An explicit target always wins, and the resolved surface is recorded on every routing decision. Dispatch is absent from the `work` tool surface, avoiding recursive Work-run dispatch.
