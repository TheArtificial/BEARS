...
| Support stalled work runs | Remaining | Continuation loss or missing tool-progress confirmation records the run as `stalled`, retains last evidence and diagnostic, and does not manufacture `failed`/`cancelled`. Operators can wait/resume when supported, cancel/end, or resolve as failed; the job projection reports unresolved stalled work. **Docket `bear_job_runs` are Bear-owned, not Pair-session-owned:** an authorized session for the same Bear may reclaim an attached current task after a defunct session, while another Bear cannot. The UI and model controls must show/cancel an active Docket run separately from sandbox `bear_work_runs`, including jobs with no sandbox runs. |
