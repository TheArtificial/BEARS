...
5. **Delete legacy.** Remove `core/letta/` and `core/codepool/` per the native migration. **In progress (2026-06).**
   ...
   - ⏳ **Phase B — structural teardown (not started; larger than anticipated).** `LettaClient`/`CodePool` are still woven through the **operator admin/management web surface**: `web/bear_management.rs` (~20 sites: Letta model catalogs, agent inspection, MemFS repo browser), `web/admin/ops.rs` (Codepool pool stats), `web/admin/bears.rs`/`admin/api.rs`/`bear_create_support.rs`/`onboarding.rs`, plus `AppState`/`ApiState` fields (`letta`, `web_letta_data`, `web_chat_transport`), `core/bears/provision.rs` (LettaClient params + MemFS role-view registration), `core/runtime/pair_turn.rs`, `startup.rs` (construction + preflight), `seeds.rs`, `api/acp/stream/sse_stream.rs`, and ~25 test initializers. Several of these are **operator features** (not dead code) — removing them is a product decision (drop vs. native re-implementation), so it should land as its own focused effort aligned with native-plan Phases 6–8, **including the compose-service removals which require explicit approval**.
