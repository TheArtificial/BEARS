...
>
> **Runtime dependency diet (2026-06): `den-protocol` extracted.** Stable runtime DTOs/contracts (`RuntimeStreamEvent`, `RuntimeSemanticEvent`, runtime refs, continuation/approval enums, error classification helpers, runtime conversation/backend traits) moved to a lightweight **`den-protocol`** crate. `den-runtime::runtime_contracts` remains as a compatibility re-export, while edge crates can import stable protocol types without depending on churny runtime internals.
>
...
| **`den-runtime`** ✅ | Native agent loop, governance/supervision modes, context assembly, role harnesses, reflection, conversation storage, bears provisioning, and runtime implementation. Defines `RuntimeToolInvoker`; stable runtime contracts live in `den-protocol` with compatibility re-exports. **Extracted (v1.4).** | `den-core`, `den-protocol`, `den-llm`, `den-memory`, `den-docket` | high |
