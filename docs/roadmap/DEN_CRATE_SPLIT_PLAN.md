...
**Remaining (true v2):** `den-core`/`den-db` split (type-only crates avoid `sqlx`), drop `async-trait` for native async-fn-in-trait (blocked where traits are used as `dyn`, e.g. `RuntimeToolInvoker`/`ToolContext`), and promote pedantic/nursery clippy from advisory to gating. The `core/tools` executors stay in the binary by design (the v1.4 `RuntimeToolInvoker` decision). **Not yet run:** DB integration/smoke tests (need the live stack) — validate with `./scripts/smoke.sh` before merge.
