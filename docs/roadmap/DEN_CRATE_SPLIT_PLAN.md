...
5. **Delete legacy.** Remove `core/letta/` and `core/codepool/` per the native migration. **In progress (2026-06).**
   ...
   - 🧹 **Deferred (non-blocking):** strip the inert `letta_configured`/`letta_api_base`/`letta_agent_*` scaffolding from `web/bear_management.rs` + `bear/details.html`; rename the retained `core/letta`-named native helpers off the `letta` name; sweep residual dead-code warnings in the clippy `-D warnings` baseline.
