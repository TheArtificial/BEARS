...
>
**Runtime dependency diet (2026-06): `den-protocol` + `den-service` extracted; `den-web` no longer depends on `den-runtime`.** Stable runtime DTOs/contracts (`RuntimeStreamEvent`, `RuntimeSemanticEvent`, runtime refs, continuation/approval enums, error classification helpers, runtime conversation/backend traits) moved to lightweight **`den-protocol`**. Concrete shared edge app state (`DenState`), Bifrost model metadata client, process-local tool-turn coordinator, active turn cancellation/controller, Bear registry/domain services, conversation persistence/read models, prompt-memory block store, memory proposal rows, pair-reflection rows, and derived recall services moved to **`den-service`**. Model option DTOs and Den model registry helpers moved to **`den-llm`**. Store-owned memory admin/read helpers moved to **`den-memory`**. `den-runtime::{runtime_contracts,bifrost,tool_turns,turn_controller,DenState,llm::model_registry,agent_assist::ModelOption,bears,conversation,prompt_memory_block_store,prompt_memory_blocks,memory_proposals,pair_reflection,recall}` remain compatibility re-exports, while web imports stable protocol/service/model/memory crates directly.
>
...
| **`den-web`** | Server-rendered admin UI + minijinja templates + template embedding (`build.rs`) + `observability/` + `s3/`. Runtime execution is injected by the root `den` binary via `WebChatRuntime`; no direct `den-runtime` dependency. | `den-service`, `den-protocol`, `den-llm`, `den-memory`, `den-docket`, `den-http`, `den-oauth`, `den-core` | medium |
...
**Goes into `den-runtime` (as originally lifted):** `agent_loop`, `native_runtime`, `runtime` (provider / `contracts` = `runtime_contracts` / `role_registry` / compaction / conversations / `turn_state` / `pair_turn` / `bearwire_projection`), the `core/llm` glue (`stream` SSE mapping), `reflection`, the runtime-side memory glue (`curation`/`curate_executor`/`bear_observations`), `work_plans` (docket projection), `sandbox`, `migration`, and the `DenToolContext` capability impls (`core/tools/*` impl side) — i.e. `den-runtime` **implements `den-tools::ToolContext`** (all 10 sub-traits) and consumes `den-docket::DocketService` + the `den-memory`/`den-llm` leaves. **Post-lift cleanup (2026-06):** stable service/protocol/storage leaves were carved back out to their owning crates: Bear registry/domain, conversation persistence/events, ACP sessions, tool-turn/turn-controller coordination, prompt-memory stores/blocks, memory proposals, pair reflection, Bifrost metadata, model registry/options, runtime contracts, and client-tool vocabulary now live in `den-service`, `den-protocol`, `den-memory`, `den-llm`, and `den-core` with direct imports from callers.
...
**Extraction order (each step keeps the workspace green + its own commit):** rename off `letta` *(done)* → den-runtime skeleton *(done)* → **runtime + shared turn/tool contracts cluster** *(done — see below)* → **runtime blob (5 SCC-ordered stages, see below)** → `DenToolContext` impls → flip `den` onto `den-runtime`, drop the flat shims → remove den-runtime compatibility re-exports after the lower crates take ownership *(done 2026-06; see cleanup note below)*.
...
**Flip complete — flat `core` shims dropped (2026-06).** The `den` binary's `core/mod.rs` previously re-exported the lifted runtime under flat aliases (`pub use den_runtime::bears;`, `pub use den_runtime::memory;`, … ~40 of them) so call sites kept resolving `crate::core::*` unchanged during the lift. The flip removed all those `pub use den_runtime::*` shims and repointed every call site directly at `den_runtime::*` (and the tool surface at `den_core::tools::*`). Mechanics: a word-boundary sweep rewrote path-form `crate::core::<rt>` → `den_runtime::<rt>`; a brace-aware splitter rewrote the nested `use crate::{ … core::{…} … }` and standalone `use crate::core::{…}` import trees, moving runtime members to a sibling `use den_runtime::{…}` while leaving den-local members (`tools`, `user`, `docket`, `work_plans`, `s3`, `email`, `sandbox`, `migration`, `api_utils`, `acp` + the `acp_runtime`/`acp_tokens`/`acp_turn_runner` aliases) under `crate::core`. The two DB integration tests in `tests/` (`acp_plan_mode`, `work_plans`) were repointed the same way (`den::core::*` → `den_runtime::*`). `core/mod.rs` now only declares the den-binary-local subsystems + the den-side ACP edge. No global `cargo fmt` was run (the tree isn't fmt-clean, so a workspace format would have been ~190 files of noise); only the touched import blocks were reformatted by hand/script. Workspace build + the `-D warnings` clippy gate (all targets) are green.

**Compatibility re-export cleanup — complete (2026-06).** After the service/protocol/storage leaves stabilized, `den-runtime` stopped advertising moved items through compatibility modules. Removed shims include `bears`, `conversation`/`conversation_*`, `acp_sessions`, `tool_turns`, `turn_controller`, `state::DenState`, `runtime_contracts`, `llm::bifrost`, `llm::model_registry`, `agent_assist::model_options`, `pair_reflection`, `memory::store`/`tools`/`admin_inspect`/`proposals`/`prompt_block_store`/`prompt_blocks`, and `client_tools`. In-repo callers now import directly from the owning crates: `den_service` (Bear/domain services, conversation persistence/events, ACP sessions, turn/tool coordination, prompt-memory, pair reflection, `DenState`, Bifrost metadata), `den_protocol` (runtime DTOs/contracts), `den_memory` (SQLite store + tools/import/admin helpers), `den_llm` (model registry/options/client DTOs), and `den_core::client_tools`. Root/edge crates gained the narrow direct dependencies this exposed (`den-memory`, `den-service`) instead of relying on `den-runtime` as an accidental umbrella. Verified with `cargo check`. **v1.4 runtime extraction is now COMPLETE without den-runtime compatibility re-export debt.**
   ...
   └── den-api   (v1/docs JSON API + create_api_app)  ── depends on ↓
        └── den-acp  (ACP protocol surface + core::acp* + ApiState + /internal)
                ...
                └── den-runtime (loop, governance, RuntimeToolInvoker + registry)
                        ↓ den-service / den-protocol / den-llm / den-memory / den-docket → den-core (+ den_core::tools + client_tools)
```
...
- `acp::tools` (`AcpToolName`, `AcpResolvedSessionPolicy`, policy/display; ~2.9k LOC) — the ACP projection of the tool surface; depends only on `den_core::tools` / `den_core::client_tools` (no longer re-exported by `den-runtime`).
