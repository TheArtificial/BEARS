...
native `async fn` in traits (drop `async-trait`), pedantic/nursery promoted from advisory to gating once clean. (`den-core`/`den-db` split was evaluated 2026-06 and deferred as low-value — feature unification means a full-workspace build compiles `sqlx` regardless; see *v1.6* and the deferral note.)
