...
- Browser web chat routes through the `chat` role and includes role metadata in the trusted Codepool `bear_channel` payload.
- ACP is being migrated to strict `pair` role routing: no Codepool fallback and no `chat`/legacy fallback. Missing `pair` must produce an operator-actionable error directing admins to provision missing role agents.
...
- Current registered runtime-critical tool groups:
  - `chat` / `pair`: `den.task.write_intent`, `den.skill.propose`, work-plan read/update/handoff tools.
...
Goal: `chat` and `pair` can capture requests for autonomous or external-effect work without executing that work directly.
...
3. Write intents only to the caller role's branch path: `chat/tasks/` for `chat`, `pair/tasks/` for `pair`.
4. Commit and push through the approved MemFS path.
5. Add tests for valid `chat` and `pair` intents, invalid schema, wrong-role access, wrong-path access, and no direct work dispatch.

Acceptance:

- A `chat` or `pair` agent can write a pending task intent through Den.
...
Goal: allowed roles can propose durable skills, while only `review` or an explicitly authorized skill-review lane can approve or reject installation into the canonical skill manifest.
...
2. Implement `den.skill.approve_proposal` and `den.skill.reject_proposal` for `review` initially, with room for a future explicitly authorized skill-review lane.
...
- `review` can approve/reject proposals with audit data.
- Approved skills update the manifest and are reflected in role reconciliation inputs.

### E. Review review runtime

Goal: `review` becomes the semantic integration and review authority described in the role model.

Tasks:

1. Add a Den-controlled review conductor, initially admin-triggered or worker-triggered.
2. Gather pending task intents, skill proposals, watch observations, and work results into a bounded review context.
3. Invoke the `review` role agent via the Letta API with only curate-appropriate Den tools.
...
- `review` can approve/reject task intents and skill proposals through Den tools.
- `review` can promote reviewed durable learnings into `core/` through Den-controlled writers.
- `review` has no outbound external communication tools.
...
3. Add a Codepool/Letta Code invocation path for the `work` role instead of reusing the hardcoded `chat` path.
...
Goal: `watch` receives inbound external events and writes observations for `review` review without taking outbound action.
...
5. Route observations into the review review queue.
6. Add tests for event validation, watch-only observation writes, no outbound action, and review review handoff.
...
3. Add end-to-end tests for `chat`/`pair` intent → `review` approval → `work` dispatch → result → `review` promotion.
4. Add end-to-end tests for `watch` event → observation → `review` review.
...
- **Channel agent** — chat or pair. The two synchronous, user-facing agents.
- **Review agent** — the integrator. Reflects on memory, approves task intents, reviews skill proposals, promotes results to `core/`.
- **Work agent** — the executor. Runs approved external-action tasks (outbound autonomous comms).
- **Watch agent** — the listener. Holds open subscriptions to external streams and produces observation events for review to review (inbound autonomous comms).
...
- Den service is running and able to chat to the Letta server.
...
2. **Skill management design.** Skills are managed by Den as Bear-scoped resources with per-role applicability. Letta Code's filesystem-based skill discovery (`.skills/`, `.agents/skills/`) applies to harness-backed agents (`chat` and `work`); API-direct agents (`pair`, `review`, `watch`) receive skills through the Letta API. These two installation mechanisms are different, and we manage both from a single canonical source rather than treating them uniformly.
   ...
   - **Agent-driven skill learning is curate-mediated.** When any agent uses `/skill` (or its equivalent), the proposed skill is captured by Den and queued as a *skill proposal* for the review agent to review during its next cycle (parallel to how task intents are reviewed). On approval, review updates the manifest; on rejection, the proposal is closed with a reason. Channel and work agents do not get raw, in-place skill installation tools — those are replaced (or wrapped) with proposal-writing tools.
   - **Reconciliation.** `reconcile_bear` validates each agent's actual installed skills against the manifest's role-relevant slice and corrects drift (re-install missing, remove extra). Drift detection mechanism differs per agent (filesystem listing for harness-backed chat/work, Letta API listing for API-direct pair/review/watch) but the canonical comparison is always against the manifest.
3. Define the Bear data model in Den's database:
   ...
   bear_agents
     bear_id (fk)
     role (talk | pair | review | work | watch)
   ...
   ```
4. Read `tasks-schema.md` and confirm tool requirements derived from it (e.g., channel agents need a privileged Den tool that can write structured intent files; the review agent needs privileged Den tools for approval/rejection and validated writes to `core/tasks/`; the work agent needs structured input handling and scoped result-writing tools for task definitions; the watch agent needs structured observation-writing tools).
...
**Goal:** Den knows what a Bear is and can chat about its five constituent agents.
...
1. Define the canonical layout per the [`memfs-sidecar-repo-views` ADR](../architecture/adr/memfs-sidecar-repo-views.md). For each Bear, the canonical bare repo has these branches:
   - `chat` — writable by chat agent. Contents: `chat/` directory, including `chat/tasks/` for intent files.
   - `pair` — writable by pair agent. Contents: `pair/` directory, including `pair/tasks/` for intent files.
   - `review` — writable by review agent. Contents: `review/` directory and `core/` directory (which contains `core/tasks/` and `core/results/`).
   ...
   - `core/` is curated shared state. The implementation must document how `core/` is materialized read-only for `chat`, `pair`, `work`, and `watch` before Phase 2 acceptance (for example read-only worktree/mount, Den-managed prompt/context copy, or Den-managed merge strategy).
...
3. Write a `pre-receive` hook for the canonical bare repo that:
   ...
   - Rejects the push if any changed path is outside that branch's allowed paths. Error message must be clear (e.g., "branch 'talk' attempted to write to 'core/notes.md'; only 'chat/' paths are allowed").
...
- Manual test: clone the `chat` branch, add a file under `core/`, push — push is rejected with the clear error.
- Manual test: clone the `review` branch, add a file under `core/`, push — push succeeds.
...
- The active `core/` materialization strategy is documented and does not give `chat`, `pair`, `work`, or `watch` raw push authority over `core/`.
...
1. Implement provisioning per role. For each role:
   ...
   - Apply the role's skill roster from the manifest. For harness-backed agents (`chat`, `work`), write the role-relevant skills to `~/.letta/agents/{agent_id}/skills/` on the harness host. For API-direct agents (`pair`, `review`, `watch`), attach skills via the Letta API.
...
2. Implement drift detection in `reconcile_bear`:
   ...
   - Skills: compare actual installed skills against the manifest's role-relevant slice.
     - For chat and work: list files in `~/.letta/agents/{agent_id}/skills/`, compute hashes, diff.
...
- A skill removed from an agent's filesystem (`chat` or `work`) or detached via the Letta API (`pair`, `review`, `watch`) is re-installed by `reconcile_bear` per the manifest.
...
**Goal:** Slack and web chat route to the Bear's chat agent via Letta Code, with no concurrency regressions and with the ability to write task intents.
...
3. Update the conversation lifecycle: each new chat session creates a Letta Conversation against the chat agent.
4. Verify the chat agent has access to the privileged Den tool needed to write task intents under `chat/tasks/` per [`../architecture/tasks-schema.md`](../architecture/tasks-schema.md). The tool should validate the intent against the schema before writing.
5. Confirm tool execution path: tools execute on the harness server (existing behavior) and don't interact with `pair/`, `review/`, `work/`, or `core/` paths in MemFS.

### Implementation note — current web-chat test slice

Before full Phase 2/3 MemFS and skill projection are complete, web chat can be tested against the `chat` role by using `bear_agents(role='talk')` as the authoritative Letta id. The retired `bears.letta_agent_id` field is not a runtime fallback. This slice intentionally does not require `pair`, `review`, `work`, or `watch` to be fully provisioned before browser chat works.

### Acceptance

- A web chat message routes correctly to the chat agent through Codepool `bear_channel` and gets a response.
...
- The push contains only `chat/` paths.
- A user can request a scheduled task ("check the deploy API every morning") and the agent writes a valid intent file to `chat/tasks/`. The intent file passes schema validation.
...
9. Verify the pair agent has the tool needed to write task intents under `pair/tasks/` per the schema. Same validation as chat agent.
...
- If a Bear has no `pair` role agent id, ACP returns a structured JSON error with request id and operator-actionable text explaining how to provision missing role agents; it must not silently fall back to `chat`.
...
## Phase 6 — Review agent and orchestration

**Goal:** review agent runs reflection, integrates learnings, reviews task intents, reviews skill proposals, and promotes work results and watch observations.

### Tasks

1. Implement the review agent's tool profile per `bear-spec.md`. It needs read access to all branches (via additional read-only worktrees) and write access to its own branch and `core/`. Reuse Letta's existing reflection and defragmentation skills. Review also needs privileged Den tools to (a) approve/reject task intents, (b) approve/reject skill proposals, and (c) update the manifest on skill approval.
2. Implement the review agent's worktree setup. The review agent reads from `chat/`, `pair/`, `work/`, and `watch/` and writes to `review/` and `core/`. Mount additional read-only worktrees for the other branches alongside the review worktree:
   ```
   ...
   ```
3. Implement review run triggering in Den:
   - Idle trigger: after N minutes of no activity on chat or pair agents.
   - Volume trigger: after M new messages across chat and pair since last cycle.
   - Pending intent trigger: when one or more files in `chat/tasks/` or `pair/tasks/` have status `pending_review`.
   ...
   - Manual trigger: an `admin/trigger_review/<bear_id>` endpoint for testing.
4. Implement review run execution. The cycle has these responsibilities, executed in this order:
   - **Memory integration:** review new content in `chat/` and `pair/` since last cycle. Promote durable learnings to `core/` through allowed review writes or privileged Den tools.
   - **Task intent review:** for each pending intent file, decide approve or reject. If approved, call privileged Den tooling to validate and write a corresponding file to `core/tasks/<task-id>.md` per the schema and update the source intent audit metadata. If rejected, call privileged Den tooling to update the source intent with rejection metadata. The review agent must not receive raw write access to `chat/` or `pair/`.
   - **Observation review:** for each new file in `watch/observations/`, decide whether the observation is salient. If yes, decide whether it warrants promotion to `core/` (a noteworthy fact for the Bear to know) or generation of a derived task intent (an observation that should trigger work — e.g., "deploy failure detected, post a summary"). Derived task intents are written by review to `core/tasks/` directly, with `parent_intent` pointing to the observation file rather than to a channel intent.
   - **Result promotion:** for each new file in `work/results/`, decide whether to surface a summary to channel agents. If yes, write a summary to `core/results/<task-id>.md` through allowed review writes or privileged Den tools.
   - **Skill proposal review:** for each pending skill proposal in `bear_skill_proposals`, decide approve or reject. On approval, call the privileged Den tool that adds the skill to `bear_skills_manifest` (with curate's chosen `applies_to_roles`) and triggers re-provisioning across affected agents. On rejection, update the proposal row with `status: rejected` and a reason. Review should consult the proposing agent's role when deciding default applicability — e.g., a skill proposed by the work agent typically applies to work only unless review sees broader value.
...
6. Ensure review runs don't run concurrently for the same Bear (Den-level lock).

### Acceptance

- Triggering a review run on a test Bear produces commits to the `review` branch touching `review/` and `core/`, with a clear commit message.
- After a review run, the next system prompt construction on the chat agent reflects content from `core/` (verify by inspecting the agent's loaded context).
- A pending intent file in `chat/tasks/` is reviewed during the next cycle; either an approval lands in `core/tasks/` and the source intent audit metadata is updated by Den, or a rejection audit update is written by Den to the source intent. The review agent never raw-writes the channel branch.
...
- Two near-simultaneous review triggers result in only one cycle running; the second is rejected or queued.
...
1. Implement a task index in Den's database, populated by polling `core/tasks/` (or, better, reacting to `post-receive` hooks on the bare repo when review pushes):
   ```
   bear_tasks
     bear_id (fk)
...
5. Implement sandboxing. The work Letta Code harness should run with:
   ...
   - No read access to `chat/`, `pair/`, `review/`, or `watch` worktrees/branches.
...
- An attempt by the work agent to read `chat/`, `pair/`, or `review/` paths fails (no read-only mount of those, plus tool roster doesn't include readers for them).
...
|------|------------------|-----------------|
| Admin bear list | `services/den/src/web/templates/admin/bears/list.html` | Show **roles** (talk / pair / review / work / watch) and **partial** provisioning; no single “Letta id” column. |
...
3. Implement subscription management. Subscriptions are durable across watch agent restarts:
   ...
   - User-requested subscriptions flow through the same task-intent → curate-approval pipeline as work tasks. A user asks chat/pair to "watch the deploy webhook"; the channel agent writes an intent; review approves; Den registers the subscription and notifies the watch agent.
...
- An attempt by the watch agent to read `chat/`, `pair/`, `review/`, or `work/` paths fails.
- An observation written by watch triggers a review run within the configured trigger window, and curate's decision (promote / generate task intent / dismiss) is recorded against the observation.
...
1. Bear-level view: lists the five agents with status, last activity, last review run, last reconcile.
...
3. Tasks view: shows pending intents, approved tasks, paused/failed tasks, and recent run history. Allows manual approval / rejection of intents (override of review decisions if needed) and manual triggering of `oneshot` tasks.
...
7. Review run log: history of cycles per Bear with timing and what was integrated/approved/promoted.
...
1. Write a migration script `den/scripts/migrate_bear.py <legacy_bear_id>`:
   ...
   - Write the legacy memory content into `core/` on the `review` branch (treating all existing memory as "already promoted").
...
2. Test the migration on a cloned production Bear in a staging environment. Verify:
   - The chat agent has access to the migrated content via `core/`.
...
2. Monitoring to put in place before wave 1:
   ...
   - **Review run success rate:** alert if cycles fail repeatedly for the same Bear.
...
3. Document operational runbooks: how to manually fix drift, how to recover from a failed review run, how to roll back a Bear, how to inspect MemFS state for debugging, how to handle a stuck or failing task, how to revoke a misbehaving work-agent capability.
...
| Agent self-modifies tools/skills out-of-band | Agents do not have tools to attach/detach tools. Skill installation is mediated by the manifest; agent-driven `/skill` learning is captured as a proposal for review review, not a direct install. The chat agent's skill directory is owned by Den; raw filesystem writes by the harness outside the manifest are detected and reverted by `reconcile_bear`. |
| Concurrent commits across worktrees | Branch-per-agent + path-per-agent enforced by `pre-receive`. |
| Review run conflicts with user load | Idle-triggered by default; volume-triggered with backoff during active hours. |
...
| Task intent backlog | Review trigger on pending intents; backlog size alert; manual override in UI. |
| Skill proposal backlog | Review trigger on pending proposals; UI surfaces the queue; operator override available. |
...
- Synchronous external work execution. Tasks are inherently async; users requesting work are told to expect a delay until the next review run approves.
