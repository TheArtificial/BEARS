...
> **Status (2026-07): the in-process Den runtime is the live stack.** BEARS runs a single **in-process Den agent runtime** for all trust stances (`chat`, `pair`, `curate`, `work`, `watch`). **Letta, Letta Code, Codepool, and the git MemFS sidecar are removed** from compose and the execution path. Bear memory/cognition is canonical in **per-Bear SQLite** ([ADR-0031](../decisions/adr-0031-sqlite-first-canonical-store-for-bear-agent-memory-and-tasks.md)); tasks/jobs target **Docket**-canonical Den Postgres ([ADR-0034](../decisions/adr-0034-jobs-and-tasks-work-management.md)). There are no Letta-runtime Bears in production, so legacy Letta/MemFS backfill is not an active roadmap item. Canonical architecture: [`../architecture/den-runtime.md`](../architecture/den-runtime.md). Canonical runtime plan: [`DEN_RUNTIME_PLAN.md`](DEN_RUNTIME_PLAN.md) (**Phases 1–6 largely landed; Phase 7 `work` sandbox implemented 2026-07 — follow-ups in [`SANDBOX_IMPROVEMENTS_ROADMAP.md`](SANDBOX_IMPROVEMENTS_ROADMAP.md)**). **[§1](#historical-§1-system-architecture-letta-era)–[Summary](#historical-summary-letta-era)** below are **Letta-era historical reference** only.
...
| Area | Status (2026-07) | Canonical docs |
|---|---|---|
...
| **Derived recall** | **Phases 0–3.5 + 5 landed** (indexer, turn recall, hybrid search, temporal + graph legs, `den reindex`); Cabinet publication/ingestion and embedding v2 deferred | [`DERIVED_RECALL_INDEX_IMPLEMENTATION_PLAN.md`](DERIVED_RECALL_INDEX_IMPLEMENTATION_PLAN.md), [`CABINET_IMPLEMENTATION_PLAN.md`](CABINET_IMPLEMENTATION_PLAN.md), [ADR-0038](../decisions/adr-0038-platform-embedding-standard-and-derived-recall-index.md) |
...
| **Docket / tasks** | **Relational Docket is canonical**: new jobs/tasks use ADR-0034 Postgres tables; current direction is one mutable conversation-linked Docket objective when a conversation invokes task orientation, with top-level tasks/subtrees projected as apparent projects. Durable Job promotion is later work. Minimal `TaskDispatcher` seam exists; legacy `bear_work_plans` service/DB/model shims and old task-list provider aliases are retired from active runtime paths. | [`DOCKET_IMPLEMENTATION_PLAN.md`](DOCKET_IMPLEMENTATION_PLAN.md), [`PHASE1_TASK_LIST_WORKFLOW_UX_PLAN.md`](PHASE1_TASK_LIST_WORKFLOW_UX_PLAN.md) |
| **Agent loop control** | **In progress**: governance/focus/orientation foundation, structured checkpoints, replayable measurement ledger, generic grounding probes, and context-budget pressure/overflow recovery are live; broader policy-driven enforcement and evidence-led tuning remain. Keep the state axes and invariants current in the living [Den state machine inventory](../architecture/den-state-machine-inventory.md). | [`AGENT_LOOP_CONTROL_IMPLEMENTATION_PLAN.md`](AGENT_LOOP_CONTROL_IMPLEMENTATION_PLAN.md), [`AGENT_LOOP_CONTROL_GROUNDING_AND_TUNING_PLAN.md`](AGENT_LOOP_CONTROL_GROUNDING_AND_TUNING_PLAN.md), [ADR-0050](../decisions/adr-0050-agent-loop-control-adaptive-budgets-and-runtime-checkpoints.md) |
...
| **Cabinet** | **Phases 0–1 landed (2026-08):** provider-neutral contract (`den-cabinet`), Postgres-backed Den facade, `cabinet_*` tools with per-Bear enablement, and the `/cabinet` wiki UI; page hierarchy + per-page policy/review (Phase 2 — a Mission is a page, not a container), attachments + recall (Phase 3), and provider ops (Phase 4) remain. Research reading/highlights are a separate ingestion client | [`CABINET_IMPLEMENTATION_PLAN.md`](CABINET_IMPLEMENTATION_PLAN.md), [cabinet contract](../architecture/cabinet-contract.md), [`CABINET_RESEARCH_INGESTION_PLAN.md`](CABINET_RESEARCH_INGESTION_PLAN.md) |
...
| **Context compaction** | **In progress**: runtime uses emergency compaction before an over-budget model call; the full Den-owned compaction lifecycle, artifacts, prompt assembly integration, and operator visibility remain to land. | [`DEN_CONTEXT_COMPACTION_IMPLEMENTATION_PLAN.md`](DEN_CONTEXT_COMPACTION_IMPLEMENTATION_PLAN.md), [ADR-0032](../decisions/adr-0032-den-context-compaction-architecture.md) |
| **Context budget tracking** | **Landed (audited 2026-07-31)**: final-request estimation, component attribution, model-limit enforcement, persisted/admin diagnostics, and provider-usage calibration are implemented. | [`CONTEXT_WINDOW_BUDGET_IMPLEMENTATION_PLAN.md`](CONTEXT_WINDOW_BUDGET_IMPLEMENTATION_PLAN.md), [ADR-0047](../decisions/adr-0047-context-window-budget-and-token-estimation.md) |
| **BearWire armature wire** | **Phases 0–3.2 complete**; BearWire HTTP edge + `bear-armature` client are implemented with opt-in/auto probing. Phase 4 switches the default and retires adapter-SSE/legacy `/acp/**` only after external Zed ACP smoke/parity validation. | [`BEARWIRE_ARMATURE_WIRE_IMPLEMENTATION_PLAN.md`](BEARWIRE_ARMATURE_WIRE_IMPLEMENTATION_PLAN.md), [`BEARWIRE_V1_PROTOCOL_REFINEMENT_ROADMAP.md`](BEARWIRE_V1_PROTOCOL_REFINEMENT_ROADMAP.md) |
...
- **Workplan ≠ semantic memory.** Activity/workplan artifacts stay in the workflow-state ontology ([`adr-0027-workflow-state-ontology.md`](../decisions/adr-0027-workflow-state-ontology.md)), not `den.memory.*`.
...
- [Agent loop control](AGENT_LOOP_CONTROL_IMPLEMENTATION_PLAN.md) — in-progress governance/checkpoint/measurement foundation
- [Agent-loop grounding and tuning](AGENT_LOOP_CONTROL_GROUNDING_AND_TUNING_PLAN.md) — in-progress companion plan
- [Context compaction](DEN_CONTEXT_COMPACTION_IMPLEMENTATION_PLAN.md) — in progress; emergency overflow recovery is wired
- [Context budget tracking](CONTEXT_WINDOW_BUDGET_IMPLEMENTATION_PLAN.md) — landed final-request estimation, attribution, enforcement, diagnostics, and calibration
