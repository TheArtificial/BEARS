...
| **Docket / tasks** | **Relational Docket is canonical**: new jobs/tasks use ADR-0034 Postgres tables; task-list projections and minimal `TaskDispatcher` seam exist; legacy `bear_work_plans` service/DB shims are retired from `den-docket` and should be treated as archived historical data only. | [`DOCKET_IMPLEMENTATION_PLAN.md`](DOCKET_IMPLEMENTATION_PLAN.md), [`PHASE1_NATIVE_PRODUCT_DEBT_PLAN.md`](PHASE1_NATIVE_PRODUCT_DEBT_PLAN.md) |
...
3. **Docket legacy compatibility retirement** — backfill or archive old `bear_work_plans` data, retire compatibility provider names from advertisement, and keep session task-list UX distinct from Docket jobs/tasks.
