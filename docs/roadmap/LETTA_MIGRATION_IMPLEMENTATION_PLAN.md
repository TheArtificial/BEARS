...
| Tool result | Yes (`tool_result`) | Yes; ACP SSE stream persists settled tool results through `CanonicalConversationRecord::tool_result(...)` in `api/acp/stream/sse_stream.rs` | Helper/unit coverage + focused ACP DB-backed validation now present in `api/acp/tests.rs` for timeout-like and error-like variants | Producer path confirmed and now explicitly validated for broader settlement payload shapes; remaining gap is mainly replay/continuation-path confirmation rather than basic timeout/error coverage |
