...
| Conversation resolved | Yes (`conversation_resolved`) | Yes; persisted from `AcpGatewayEvent::ConversationResolved` side effects in `api/acp/stream/runtime.rs`, and emitted as an initial ACP event in `api/acp/stream/orchestration.rs` when a resolved conversation is known | Now has focused ACP DB-backed validation in `api/acp/tests.rs` (`acp_conversation_resolved_side_effect_persists_canonical_workflow_event`) | Better than earlier audit suggested: path is wired and now explicitly validated for canonical persistence; remaining question is mainly whether this record is transcript-read-critical or primarily provenance/debug support |
