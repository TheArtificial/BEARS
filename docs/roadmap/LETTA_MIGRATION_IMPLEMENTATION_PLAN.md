...
Status: **shared audit layer hardening expanded across test homes**
- `core/conversation_events.rs` now has focused shared-layer tests covering:
...
- `api/acp/mod.rs` test coverage now also validates real-consumer payload conventions for the shared non-ACP audit projection layer, including:
  - memory-curate enqueue workflow payload shape
  - visible-summary policy behavior for completion-style projections
- Canonical-focused test coverage is now 30 passing tests.

The next broader non-ACP slice should likely move from helper-shape coverage into **true persistence/integration hardening**, for example:
- expanding from payload-shape tests into persistence-level behavior for migrated consumers,
...
| --- | --- | --- | --- | --- |
| Visible user message | Yes (`visible_user_message`) | Yes; ACP prompt flow now persists user prompts through canonical visible-message persistence in `api/acp/stream/prompt_flow.rs` | Indirect history behavior tests + canonical helper coverage + focused ACP DB-backed validation in `api/acp/tests.rs` for provenance/dedup semantics | Prompt-path canonical persistence now has explicit provenance/dedup validation, including dedup for identical prompt provenance and separation for distinct request_ids; remaining gap is narrower and mostly about broader end-to-end prompt-path coverage rather than persistence semantics |
...
| Tool result | Yes (`tool_result`) | Yes; ACP SSE stream persists settled tool results through `CanonicalConversationRecord::tool_result(...)` in `api/acp/stream/sse_stream.rs` | Helper/unit coverage + focused ACP DB-backed validation for timeout-like/error-like payloads + explicit continuation-mapping tests + resumed-stream gating coverage in `api/acp/tests.rs` | Producer, continuation mapping, and resumed-stream terminal gating are now explicitly validated for timeout/error payloads, `timeout`/`timed_out` normalization, and approval-decision conversion; remaining gap is narrower and mostly about deeper live-runtime integration breadth rather than tool-result seam uncertainty |
| Conversation resolved | Yes (`conversation_resolved`) | Yes; persisted from `AcpGatewayEvent::ConversationResolved` side effects in `api/acp/stream/runtime.rs`, and emitted as an initial ACP event in `api/acp/stream/orchestration.rs` when a resolved conversation is known | Focused ACP DB-backed validation in `api/acp/tests.rs` for canonical persistence plus lighter replay/read validation confirming the workflow record does not pollute visible transcript history projection | Better than earlier audit suggested: path is wired, canonically persisted, and now lightly validated against replay/read behavior; remaining question is mainly whether this record is transcript-read-critical or primarily provenance/debug support |
| Turn outcome / terminal result | Yes (`turn_outcome`) | Yes; ACP SSE stream persists terminal outcome on terminal turn emission in `api/acp/stream/sse_stream.rs` | Strong helper/unit coverage + focused ACP DB-backed validation now present in `api/acp/tests.rs` for failed, cancelled, and recovered terminal payloads | Primary workflow-event path now has explicit branch coverage for failure/cancellation/recovery-style persistence; remaining gap is narrower and mostly about any additional rare terminal modes rather than missing core branch proof |
...
- **Highest-value remaining validation gaps before more coding:**
  1. fuller stack-level smoke/integration confirmation that replay and continuation semantics hold against the full ACP gateway, not just the fast DB-backed lib layer,
  2. any additional rare terminal/runtime edge-path confirmation that proves important during migration testing.
