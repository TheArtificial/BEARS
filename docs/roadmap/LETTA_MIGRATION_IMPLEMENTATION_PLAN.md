...
| Tool result | Yes (`tool_result`) | Yes; ACP SSE stream persists settled tool results through `CanonicalConversationRecord::tool_result(...)` in `api/acp/stream/sse_stream.rs` | Helper/unit coverage + focused ACP DB-backed validation for timeout-like/error-like payloads + explicit continuation-mapping tests in `api/acp/tests.rs` | Producer and continuation mapping paths are now explicitly validated for timeout/error payloads and approval-decision conversion; remaining gap is narrower and mostly about fuller end-to-end resumed-stream coverage rather than missing canonical/result-shape proof |
