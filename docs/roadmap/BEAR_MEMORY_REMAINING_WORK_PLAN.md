...
| **6 — Tools + governance** | `entity_browse`, `entity_resolve`; descriptive relation writes from roles; `entity_merge`/`split` + access-rule writes **curate-only**; resolved entities in `session_info` | ◻ |
