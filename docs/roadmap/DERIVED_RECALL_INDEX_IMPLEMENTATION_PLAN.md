...
- ✅ Unified provenance: each hit is `{ memory_id, path, kind, score, snippet, source }` (`source` ∈ `vector`/`keyword`; `score` null for the unranked keyword leg), with a top-level `strategy` (`hybrid` | `vector` | `keyword`, reflecting which legs contributed), `storage: "hybrid"`, and the vector leg's recall `diagnostic`. Results de-dupe by `memory_id` and cap to the requested limit, prioritizing vector hits.
