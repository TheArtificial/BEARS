...
- ✅ Indexing policy per ADR-0038: `visibility = normal`; `shared` records indexed; `profile_local` limited to `note`/`decision`/`summary`; `scratch`/`log` excluded. Head-only selection uses canonical head queries and respects store-level supersession writes.
...
> **Follow-ups (not blocking):** live worker exercise end-to-end (rebuilt Den + a real embedding key) is pending. Curate/consolidation still needs policy logic for when to supersede; the memory store can already write supersession and invalidate predecessors.
...
- ✅ Unified provenance: each hit is `{ memory_id, path, kind, score, snippet, source, salience }` (`source` ∈ `vector`/`keyword`; `score` null for the unranked keyword leg), with a top-level `strategy` (`hybrid` | `vector` | `keyword`, reflecting which legs contributed), `storage: "hybrid"`, and the vector leg's recall `diagnostic`. Results de-dupe by `memory_id` and cap to the requested limit, prioritizing vector hits. Vector scores apply the current salience multiplier from ADR-0041.
