...
The useful-minimum measurement spine is now live: the transcript-free ledger records checkpoint, context-budget, and grounding-probe decisions; runtime replay helpers support typed observations, turn/profile expected-vs-observed comparisons, reason counts, and DB-backed profile summaries. Generic MVP grounding probes are wired into mutation verification, context pressure participates in turn-budget state, emergency compaction runs before over-budget model calls, checkpoint responses are authoritative in enforce mode, and initial checkpoint/KO cadence tuning has started from recorded/live behavior. Remaining work should stay evidence-led: add stronger probes, checkpoint-before-growth, or more replay machinery only when recorded runs show the current hooks are too weak.
