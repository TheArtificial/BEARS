...
3. **Structural safety, not prompt safety** ([bear-stances.md](../architecture/bear-stances.md), [ADR-0039](../decisions/adr-0039-trust-profiles-and-governance.md)). No stance combines untrusted input, outbound action, and shared-memory write; boundaries are Den-enforced policy, not system-prompt requests. This matches the strongest current thinking on agent security (the "lethal trifecta" framing and Meta's Agents Rule of Two), and the trust-profile × governance split in ADR-0039 resolves a conflation most platforms haven't even noticed yet.
...
| Supervision is orthogonal to trust | Client disconnect is a governance transition, not a silent `pair`→`work` flip that would change memory scope mid-session | [ADR-0039](../decisions/adr-0039-trust-profiles-and-governance.md) |
| Effective policy is computed, never inferred by the model | `TrustProfile × Governance × Armature × RunAuthContext`; Den renders the one applicable instruction pre-inference | [ADR-0039 §3](../decisions/adr-0039-trust-profiles-and-governance.md), [context-compilation-scenarios.md](../architecture/context-compilation-scenarios.md) |
...
**1. New capability? Attach it to an existing stance.** A sixth stance needs a genuinely distinct combination of surface, trust boundary, memory pattern, communication posture, and product meaning ([bear-stances.md § future roles](../architecture/bear-stances.md#future-roles)). Most proposals that feel like "a new stance" are actually a new *channel*, a new *armature*, a new *governance*, or a new *tool roster* — the axes in [interactive-stances-and-role-axes.md](../architecture/interactive-stances-and-role-axes.md) absorb them. Note ADR-0039's own open question of whether `chat`/`pair` eventually collapse into one `interactive` trust profile distinguished by armature — the axes model is the direction of travel; don't add enumeration where an axis exists.
...
**6. New autonomy? It flows through Docket with acceptance criteria, under a governance.** Anything that acts without a human present needs: an approved task with concrete `completion_criteria` ([ADR-0034](../decisions/adr-0034-jobs-and-tasks-work-management.md)), a governance with defined transitions ([ADR-0039](../decisions/adr-0039-trust-profiles-and-governance.md)), and agent-loop-control limits ([ADR-0050](../decisions/adr-0050-agent-loop-control-adaptive-budgets-and-runtime-checkpoints.md)). If a feature wants the model to keep working "until done" without a criteria object, it's missing its stopping condition. Consider adding per-Bear daily cost/action budgets as an operational backstop — turn-scoped budgets don't bound a scheduler that keeps dispatching turns.
