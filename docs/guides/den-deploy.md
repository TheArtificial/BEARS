...
| `API_SERVER_URL` | Public API origin when `RUN_API=true`; for Den armature adapters this may be `https://api.bears.[domain]`, another hostname, or a published host+port URL. The API listener mounts ACP compatibility routes under `/acp/*` and BearWire under `/bearwire`. |
