...
**Templates in production:** non-`production` builds load MiniJinja from the filesystem (`TEMPLATES_DIR`). Release / `--features production` **embeds** templates at compile time—editing files on disk is not enough; rebuild the binary (see [`den-quickstart.md`](den-quickstart.md)).
...
Optional query **`conversation_id`** — when set (for example `conv-…` from bear details links), the page script selects that Den conversation in `#den-conv` when it appears in `GET /v1/chat/conversations` (otherwise the first listed thread, or a single **Main chat** / `default` option).
...
4. **Den native SSE stream** — `POST /v1/chat/send` returns SSE from the in-process agent loop; the chat handler uses `connect.handler` + `connect.stream` to parse `data:` JSON lines (implemented as `lettaSseHandler` in `bear_chat.html` — historical name). It surfaces **`error_message`** (including nested `contents`) as stream-safe **`text` + `role: error`**; streams **`reasoning_message`** into a dedicated **HTML** ai bubble (single-line horizontal scroll, optional Expand for full text — plain text only, no Markdown in that bubble per Deep Chat’s `html` vs `text` tradeoff, see [deep-chat#429](https://github.com/OvidijusParsiunas/deep-chat/issues/429)); then **`assistant_message`** as **`text`** (Markdown-friendly). **Assistant text is accumulated for the whole response** (do not reset the buffer when `id` changes between stream chunks, or the UI shows a truncated prefix while persisted history has the full reply). **`stripSystemReminderBlocksLive`** removes only **closed** `<system-reminder>` blocks while streaming; a final pass on stream end can apply the full strip (incl. partial unclosed tag at EOF). **`overwrite: true`** for each chunk after the first assistant chunk (or when replacing **Thinking**).

5. **Errors & support refs** — Non-OK responses use JSON `{ "error", "request_id" }` with header **`X-Request-Id`**. The template formats user-facing strings with `formatChatError` (title, detail, reference). **`messageStyles.default.error`** styles connect error bubbles with **`--error-color`** and a left accent border. Empty streams and malformed SSE lines get explicit error bubbles instead of failing silently. **`errorMessages.displayServiceErrorMessages: true`** must be in effect **before** Deep Chat builds its internal message stack: set it on **`<deep-chat errorMessages='{"displayServiceErrorMessages":true}'>`** in markup (and `configureDenChat()` re-applies the same for consistency). **Boot order:** `loadConversations()` runs **first** (so `CONVERSATION_ID` and `#den-conv` match `?conversation_id=` and history fetches the same thread), then **`customElements.whenDefined('deep-chat')`**, then **`configureDenChat()`** — the markup `errorMessages` attribute is enough to avoid a silent `onResponse({ error })` even though configuration follows the conversations fetch. For **`connect.stream: true`**, do **not** use `{ error: string }` on `signals.onResponse` — that triggers `streamError`/`finaliseStreamedMessage` with no prior `{text}`/`{html}` chunk and throws **“No valid stream events”**. Use **`{ text: string, role: 'error' }`** instead (`denChatSignalError`, stream error helpers in `bear_chat.html`).

6. **Metrics** — Den exposes **`GET /metrics`** (Prometheus text, in-process counters for chat send). Intended for internal scrape targets only.

**Local dev:** After changing anything under `src/web/assets/`, rebuild and restart `den` so `memory-serve` picks up routes and paths ([`den-quickstart.md`](den-quickstart.md) § *Static assets*).
