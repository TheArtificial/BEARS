...
The schema is applied automatically on startup from `migrations/`. For **`SQLX_OFFLINE`** / CI builds, run `./scripts/sqlx.sh prepare --workspace -- --all-targets` and commit [`services/den/.sqlx/`](../../services/den/.sqlx/). CI verifies it with `./scripts/sqlx.sh migrate run` then `./scripts/sqlx.sh prepare --check --workspace -- --all-targets`. See [sqlx-patterns.md](sqlx-patterns.md).
