...
When current docs disagree, treat this document as the source for cross-functional role meaning and messaging, ADRs as the source for historical architectural rationale, and runtime specs as the source for implementation behavior.
...
`chat` is the Bear role people meet in chat. It handles synchronous conversation in Slack, web chat, Discord, and similar text-in/text-out channels.
...
`review` is deliberately not user-facing. It exists so the Bear can learn, remember, approve, reject, summarize, and integrate without giving every outward-facing role broad authority over shared memory or autonomous action.
...
`watch` is the Bear role for inbound external events. It receives webhooks, polling results, queue messages, subscription updates, and other external signals, then writes structured observations for `review` to review.
...
6. `review` reviews memories, task intents, observations, skill proposals, and work results.
7. `review` promotes durable knowledge into `core/` and uses Den-controlled tools to approve or reject cross-role changes.
8. Den dispatches approved external tasks to `work`.
9. `work` executes within its approved scope and writes results.
10. `review` reviews those results and promotes durable learnings back into `core/`.

In short:

- `chat` and `pair` are the synchronous user-facing roles.
- `watch` is the inbound external-events role.
- `review` is the semantic integration and review role.
...
- `review` is merely a summarizer; it is the semantic integration and review authority.
- “Space” is the primary conceptual layer users need to understand.

### User-facing naming

The role names `chat`, `pair`, `review`, `work`, and `watch` are the preferred stable vocabulary.
...
- `chat` and `pair` should normally speak in the voice of the Bear, not as “the chat role,” “the pair role,” or a separate agent.
...
- Durable shared memory should be explainable as something `review` promoted, not something every role writes freely.
