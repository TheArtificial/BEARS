...
> **Direction changed (2026-06).** The Bear-facing memory model below stands, but its storage is changing: canonical Bear memory moves to **per-Bear SQLite** ([ADR-0031](../decisions/adr-0031-sqlite-first-canonical-store-for-bear-agent-memory-and-tasks.md)), the git MemFS sidecar and Letta Archives are removed, and the `core/`/role-branch file tree becomes a logical-path projection over SQLite records. See [Den-Native Runtime: memory model](den-native-runtime.md#memory-model-under-sqlite) and the [migration plan](../roadmap/DEN_NATIVE_RUNTIME_PLAN.md).
