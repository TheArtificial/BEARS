...
**Related:** [`bear-roles.md`](bear-roles.md), [ADR-0006 — Bear work surfaces](../decisions/adr-0006-bear-work-surfaces.md), [ADR-0039 — Trust profiles and governance modes](../decisions/adr-0039-trust-profiles-and-governance-modes.md)
...
| **Trust profile** | Which memory is writable and what approval class applies? | **Trust profile** (`Profile` in code) | pair-local writes, client approval, no raw `work/` reads |
...
**Trust profile** is the slow-changing trust-and-memory contract; **governance mode** is the fast-changing supervision dial on a run. They are orthogonal — see [ADR-0039](../decisions/adr-0039-trust-profiles-and-governance-modes.md). The runtime computes, per turn:
...
## Trust profiles (post-Letta)

**Trust profiles** are durable trust contracts over one Den-native agent loop — not separate Letta agents or harness processes. Documentation says **trust profile**; the code shorthand stays `Profile` (`BearProfile`, `bear_profile_bindings.profile`).

| Trust profile | Human present? | Primary trust boundary | Typical armature |
|---------------|----------------|------------------------|------------------|
...
A trust profile is applied **per turn** as a template. It is **not** the identity of a long-running sandbox or run; that lifetime belongs to the workspace session and its governance-mode timeline ([ADR-0039](../decisions/adr-0039-trust-profiles-and-governance-modes.md)).
...
**Handoff seams** (remote `pair` going offline, interrogating a long `work` run, panic/checkpoint) are governance-mode transitions on a stable run + workspace session — not trust-profile flips.

## Non-interactive trust profiles (keep distinct)

`curate`, `work`, and `watch` remain separate trust profiles because they encode **non-negotiable trust boundaries** (lethal trifecta split), not channel choice or supervision state:
...
They are trust profiles, **not** governance modes of an interactive session.

## Implementation direction

1. Treat **`chat` and `pair` as interactive trust profiles**; channel choice selects armature template, not a second Bear.
2. Attach **work surfaces to threads/sessions**, resolved from anchors and hints — not “owned by pair.”
3. Model **governance mode** on the run / workspace session; do not flip trust profile to express "human went offline."
4. Retire harness/runtime-family language (`letta_code_harness`, five provisioned “agents”).
5. Prefer **trust profile** (`Profile` in code, Postgres registry `bear_profile_bindings`) and **governance mode** (`Mode` in code) in operator docs.

When `AGENT_RUNTIME=native`, `chat` and `pair` are trust profiles over one Den loop; work-surface **binding** is conversation-scoped. See [`../guides/work-surfaces-and-conversations.md`](../guides/work-surfaces-and-conversations.md).

## Naming

The implementation enum (`BearProfile`) and Postgres profile registry use **`chat`** (formerly `talk`) for the conversational human-present trust profile, aligned with product language in [`bear-roles.md`](bear-roles.md). "Trust profile" is the documentation term for the `Profile` concept; "governance mode" is the documentation term for the `Mode` concept ([ADR-0039](../decisions/adr-0039-trust-profiles-and-governance-modes.md)).
