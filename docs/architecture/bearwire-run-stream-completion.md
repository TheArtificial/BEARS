...
not, by itself, a terminal run outcome. The persisted canonical run state, including
unresolved client obligations, decides whether an adapter may end its prompt; the
stream is transport and projection, not the authority on run completion.

| Observed boundary | EOF policy | Durable run/session result |
| --- | --- | --- |
| `run.completed`, `run.failed`, `run.cancelled`, or `error` | Finish normally. | Persist the matching terminal run state. |
...
- On a missing terminal event, adapters decide whether they may end the prompt from
  canonical persisted run state and obligations after the bounded reconciliation;
  they must not infer a terminal failure from stream EOF alone.
...
- **terminal run event**: `run.completed`, `run.failed`, `run.cancelled`, or `error`.
