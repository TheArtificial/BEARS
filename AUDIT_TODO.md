...
- [x] `src/oauth/endpoints.rs:1203-1229` vs `auth.rs:38-69` — two near-identical `extract_bearer_token` impls with subtly different validation (one trims/checks empty, other doesn't) — unify. **DONE** (duplication batch): endpoint now uses `auth::extract_bearer_token_oauth`, a thin OAuth-error wrapper around the single auth parser; local duplicate parser removed; `den-oauth` clippy green.
...
- [x] `src/v1/user.rs:198-246` `login` nests 3-deep if-let with duplicated `Err(CustomError::Authentication(...))` in two branches — flatten. **DONE** (handler cleanup): uses early returns plus a shared invalid-credentials closure; success path is no longer nested; `den-api` clippy green.
...
- [x] `src/email/mod.rs:105-107` manual "split before first dot" for `type_name` with no comment explaining significance. **DONE** (docs batch): added comment explaining persisted message type records the logical template family independent of extension/renderer; `den-http` clippy green.
...
- [x] `plan_mode.rs:250-279` `get_for_session` builds two different SQL strings via if/else with conditional `.bind()` arity — error-prone, split into two named query fns. **DONE** (clarity batch): split into `get_for_session_by_id` and `latest_for_session`, each with fixed SQL/bind arity; `den-runtime` clippy green.
- [x] `plan_mode.rs:420-495` `approve_plan_mode`/`reject_plan_mode`/`cancel_plan_mode` duplicate the "fetch current, decide client_session_id" preamble verbatim. **DONE** (duplication batch): extracted `current_plan_mode_for_close` and `effective_client_session_id`; `den-runtime` clippy green.
- [x] `plan_mode.rs:497-513` `close_with_state` runtime-guards invalid states via `matches!` — model as a "ClosedPlanModeState" enum to make invalid states unrepresentable. **DONE** (typing batch): added private `ClosedPlanModeState` enum carrying only approved/rejected/cancelled and deriving storage/event values; `den-runtime` clippy green.
...
- [x] `agent_assist/conversation_title.rs:329-365` `looks_like_machine_or_opaque_title` — 37-line function with 5 heuristics, split into named sub-checks. **DONE** (clarity batch): extracted named checks for conversation-id matches, UUID/conv UUID titles, legacy chat-id fallback, and long hex tokens; `den-runtime` clippy green.
- [x] `client_obligation_coordinator.rs:88-162` `existing_tool_result_or_late`/`existing_permission_result_or_late` structurally identical except outcome type — share via generic helper. **DONE** (duplication batch): added shared `existing_client_result_or_late` returning an internal outcome enum, then mapped to tool/permission outcomes; `den-runtime` clippy green.
- [x] `client_obligation_coordinator.rs:412-482` `settle_permission_result` god function — extract `dispatch_local_tool_after_grant(...)`. **DONE** (structure batch): armature-local dispatch after granted permission now lives in `dispatch_local_tool_after_grant`; `den-runtime` clippy green.
- [x] `client_obligation_coordinator.rs:450-459` raw `Value` navigation for tool_name/arguments instead of a typed payload struct. **DONE** (typed-boundary batch): added `LocalToolRequestPayload` deserialization and used it for armature-local tool dispatch; `den-runtime` clippy green.
...
- [x] `agent_loop/assembler.rs:113-121` dead/always-true ternary (`if selected_paths.is_empty() {"available"} else {"available"}`) — leftover copy-paste. **RECONCILED**: current `projected_memory_session_diagnostic` sets status directly and no longer contains the dead ternary; verified in code.
...
- [x] `src/promotions.rs:42-72` `promote_to_shared_core` returns unlabeled `Result<(String, String), DenError>` tuple — use a named struct. **DONE** (typed-boundary batch): added `CorePromotionOutcome { memory_id, promotion_id }` for den-memory promotion APIs; den-runtime wrappers preserve their existing tuple API; `den-memory`/`den-runtime` clippy green.
...
- [x] `src/relations.rs:210` `descriptive_entity_ids_by_source` is dead public API — never re-exported/called outside its own test. **RECONCILED**: finding was stale/incorrect; function is used by `den-service::recall::query::graph_expand_hits` and `den-service::recall::reconcile::list_indexable_heads`, so it remains live.
...
- [x] `src/descriptors.rs`, `logical_path.rs`, `entity.rs` — hand-written `as_str`/`parse` pairs for `RelationClass`/`EntityTrust`/`ResolutionState`/`MemoryScopeType` instead of `Display`/`FromStr`. **DONE** (typing batch): implemented `Display`/`FromStr` for `RelationClass`, `EntityTrust`, `ResolutionState`, and `MemoryScopeType`; existing `parse` helpers now delegate to `.parse()` where present; `den-memory` clippy green.
- [x] 6+ files (`entity.rs`, `harvest.rs`, `promotions.rs`, `proposals.rs`, `reflection_outcomes.rs`, `records.rs`) — RFC3339 "now" formatting boilerplate duplicated verbatim; a `now_rfc3339()` helper already exists in `entity.rs`/`relations.rs` but isn't shared. **DONE** (duplication batch): added shared `clock::now_rfc3339()` and reused it across entity, relations, harvest, observations, promotions, proposals, records, reflection outcomes, and legacy import; `den-memory` clippy green.
- [x] `src/records.rs:763-781` `impl AsOfSqlRow` placed after a `#[cfg(test)]` block rather than near its struct def (line 561) — breaks file's ordering convention. **DONE** (ordering batch): moved `impl AsOfSqlRow` directly below the struct definition before the test module; `den-memory` clippy/tests green.
- [x] `src/proposals.rs` — uses raw positional tuples as query row types instead of a named `FromRow` struct like sibling files — `row.0`/`row.1` indexing is hard to follow. **DONE** (clarity batch): added `ProposalSqlRow` and `ConsolidationCandidateSqlRow` with `FromRow`; removed positional proposal tuple decoding; `den-memory` clippy green.
...
- [x] `src/logical_path.rs:77-117` `from_logical_path` duplicates work_surfaces extraction logic twice (shared vs profile-local paths) — factor into one helper. **DONE** (duplication batch): extracted `parse_work_surface_path` and reused it for shared/profile-local work surface paths; `den-memory` clippy green.
...
- [x] `src/tools/work_surface/mod.rs:229-246` `work_surface_scaffold_paths` returns unnamed 5-tuple destructured positionally at both call sites — use a named `WorkSurfacePaths` struct. **DONE** (clarity batch): added private `WorkSurfacePaths` and updated scaffold request construction to use named fields; `den-core` clippy green.
- [x] `src/tools/web/mod.rs:47-89` `web_fetch` — "blocked" and "not approved" branches duplicate identical 12-line `record_fetch_attempt` call — factor into one audit-then-return helper. **DONE** (duplication batch): added `record_unexecuted_fetch_attempt` shared by blocked/not-approved branches; `den-core` clippy green.
...
- [x] `src/tools/environment/payloads.rs:45-60` — clones full `Option<Value>` just to read a few fields; `.as_ref()` + shadowing would avoid the clone in a hot path. **DONE** (cleanup batch): optional memory diagnostics are borrowed with `.as_ref()` before cloning only the selected `Value`; `den-core` clippy green.
...
- [x] `src/methods/client.rs:146-157` `ClientToolResultRequest::input()` clones 5 fields incl. a `Value` — avoid by consuming `self` or borrowing. **DONE** (ownership cleanup): replaced `input(&self)` with consuming `into_input(self)` and moved finish-payload construction to borrow `ClientToolResultInput`; `den-bearwire` clippy green.
...
- [x] `src/auth.rs:7-21` vs `events.rs:55-62` — duplicated "read Authorization header → strip → trim → validate" pattern — share one combinator. **RECONCILED**: current `events_page` delegates to `authenticate_for_bear_slug`, which uses the single `bearer_token` helper in `auth.rs`; no duplicate parser remains.
- [x] `src/methods/session.rs:429` — `let _ = ...append_bearwire_event(...).await?;` discards `event_sequence`; the only handler in the file that doesn't surface `event_sequence` in its response, inconsistent with siblings. **DONE** (consistency batch): model-selection response now includes `event_sequence`; `den-bearwire` clippy green.
- [x] `src/methods/mod.rs:16-32` `initialize_result` hardcodes route strings (`"/bearwire/v1/rpc"`) that don't match actual mounted routes in `lib.rs:11-12` (`/v1/rpc`) — stale/misleading self-description payload. **DONE** (correctness batch): initialize payload now advertises `/v1/rpc` and `/v1/sessions/{session_id}/events/page`; `den-bearwire` clippy green.
...
- [x] `src/methods/run.rs:203-211` `ResolvedRunModel.source: &'static str` is a stringly-typed log-only tag — use an enum with `Display`. **DONE** (typing batch): added `ResolvedRunModelSource` enum with `Display` and replaced source string literals; `den-bearwire` clippy green.
...
- [x] `src/methods/mod.rs` — no crate/module-level `//!` docs anywhere explaining BearWire's purpose/protocol shape. **DONE** (docs batch): added module docs describing transport ownership and method handler responsibilities; `den-bearwire` clippy green.
...
- [x] `src/user/settings/email.rs:64,86,118,169,193` — same `auth_session.user.clone().unwrap().id` repeated 5x — extract shared helper returning `Result<_, CustomError>`. **DONE** (safety/duplication batch): added `session_user_id(&AuthSession) -> Result<i32, CustomError>` and replaced all five unwraps; also propagated verification-token session read errors instead of unwrapping; `den-web` clippy green.
...
- [x] `src/core/s3/mod.rs:37,51,59,68` — four `.expect()` calls in startup config parsing, undocumented as fail-fast-only. **DONE** (docs batch): documented `MediaStore::new` as startup configuration parsing where non-empty invalid S3 settings intentionally fail fast; `den-web` clippy green.
- [x] `src/v1/mod.rs:826,838,906,919` — `direct_chat_sse_response`/`chat_sse_response`/header-building block all duplicate identical `Response::builder()...text/event-stream` boilerplate 3x — share one builder. **DONE** (duplication batch): added `chat_sse_body_response(Body, request_id)` and reused it for direct and streaming SSE responses; `den-web` clippy green.
...
- [x] `src/admin/oauth_clients.rs:331-361` — near-identical copy-pasted error-extraction closures differing only by field name — extract `field_error_messages(errors, field)`. **DONE** (duplication batch): added `field_error_messages` helper and reused it in the add-client validation path; `den-web` clippy green.
- [x] `src/bear/settings.rs:475-477` `pretty_json` swallows serialization errors via `unwrap_or_else` with no comment. **DONE** (docs batch): added fallback rationale explaining admin diagnostics should still render if pretty formatting fails; `den-web` clippy green.
...
- [x] `src/lib.rs:314-323` `/metrics` handler uses a fallible `Response` builder for a fixed content-type/body that can't really fail — overkill. **DONE** (cleanup batch): metrics route now returns an infallible `(headers, body)` tuple response; `den-web` clippy green.
- [x] `src/bear/memory.rs:26-46` `is_ephemeral_progress_status`/`strip_ephemeral_status_suffixes` — loop-based suffix stripping with no comment on termination guarantee. **DONE** (docs batch): functions now live in `observability/chat_proxy_stream.rs`; added termination guarantee to the suffix-stripping doc comment; `den-web` clippy green.
...
- [ ] `src/conversation/events.rs`, `message_types.rs`, `bears/managed_blocks.rs`, `bifrost_governance.rs` — 10 enums hand-implement `as_str(self) -> &'static str` instead of `Display`/`AsRef<str>`. **PARTIAL** (typing batch): added `Display` impls for `ConversationMessageType`, `ConversationMessageVisibility`, and `ConversationMessageRole`; remaining enum families still need follow-up.
- [x] `src/memory_proposals.rs:7-9` — pointless doubled brace nesting in a `use` statement — unreviewed rustfmt/import churn. **RECONCILED**: current imports are already flat after rustfmt/import cleanup; no code change needed.
...
- [x] `src/tool_turns.rs:130-136,518-523` — converts `PendingToolTurn`/`ToolTurn` to `SettledToolResult` field-by-field with 5-6 clones — add a `From<&ToolTurn> for SettledToolResult` impl. **DONE** (conversion batch): added `From<&ToolTurn> for PendingToolTurn` and `From<(&ToolTurn, &ToolResultRequest)> for SettledToolResult`, then reused both in pending/settled conversion sites; `den-service` clippy green.
...
- [x] `src/conversation/events.rs:1206-1211` — two `.expect()`s assume `ProjectionEvent` always serializes to a JSON object — safe but undocumented invariant. **DONE** (safety/observability batch): projection serialization now handles non-object/serialization failures with warning logs and fallback JSON instead of panicking; `den-service` clippy green.
...
- [x] `src/native_runtime/turn.rs:274-275,1032-1033,1076-1077` — `serde_json::from_str(...).unwrap_or_else(|_| Value::Object(Default::default()))` duplicated 3+ times — extract `parse_args_or_empty_object`. **DONE** (cleanup batch): added `parse_args_or_empty_object` and reused current native-turn fallback parses; `den-runtime` clippy green.
...
- [x] `src/gateway_events.rs:668-750` — 4 nearly identical field-lookup helpers (`tool_call_value`/`tool_call_id`/`tool_call_name`/`tool_call_args_raw`) — replace with one generic `lookup_field(&[&Value], &str)`. **DONE** (lookup batch): added shared nested-field lookup for tool-call id/name/arguments; `den-runtime` clippy green.
...
- [x] `src/gateway_events.rs:1063` `preview_str_truncated` — byte-slice `&s[..max]` can panic on multi-byte UTF-8 boundaries — use `char_indices`/`floor_char_boundary`. **RECONCILED**: current code backs off to a UTF-8 char boundary before slicing; verified in `gateway_events.rs`.
...
- [x] `src/reflection/conductor.rs:1216-1231` `parse_context_compact_input` returns `Result<_, String>` instead of `DenError`, inconsistent with rest of file. **RECONCILED**: intentionally returns the DB failure message string passed directly to `mark_context_compact_failed(&error)`; converting to `DenError` would only re-stringify at the call site.
...
- [x] `src/agent_loop/context.rs:401,405` `load_transcript_grouping_rows` — `.unwrap_or_default()` on a DB fetch error silently returns empty transcript instead of propagating, masking real failures as "no history." **DONE** (correctness batch): DB fetch errors now propagate with `?`; `den-runtime` clippy green.
...
- [x] `src/llm/stream.rs:553,674,627` — 3 separate `from_utf8(...).map_err(...)` blocks with copy-pasted error text — extract `decode_utf8_or_system_error`. **DONE** (duplication batch): added `decode_utf8_or_system_error` and reused it for OpenAI/Responses SSE frame decoding; `den-runtime` clippy/tests green.
...
- [x] `src/runtime/compaction/lifecycle.rs:301-321` `prepare_turn_compaction` — labeled "back-compat alias" with 3 duplicate match arms all calling the same function — collapse to `_ => ...`. **DONE** (cleanup batch): only `TurnStart` is special-cased; all other triggers use the shared job path; `den-runtime` clippy green.
...
- [x] `src/turn_waits.rs:72-162,164-445` `persist_surface_obligation_transactionally`/`persist_bearwire_tool_call_wait_transactionally` — duplicate an entire tx block (begin/update state/select-or-insert step/update to waiting) verbatim — extract `ensure_waiting_turn_step(...)`. **DONE** (transaction helper batch): extracted `ensure_waiting_turn_step` with fixed step SELECT/INSERT/update SQL and reused it in both transactional wait paths; `den-runtime` clippy green.
...
- [x] `src/turn_waits.rs:256-354` — 3-way branching (UPDATE-RETURNING / INSERT-ON-CONFLICT-RETURNING / separate INSERT) with the `RETURNING` column list repeated verbatim 3x — extract a shared constant. **DONE** (SQL cleanup): added shared obligation/step column constants and reused them in the wait persistence queries; `den-runtime` clippy green.
- [x] `src/turn_waits.rs:10-26` `PersistToolCallWaitInput` — several fields typed `&'a Option<String>` rather than the more idiomatic `Option<&'a str>`, forcing callers to hold owned `Option<String>`s. **DONE** (typing batch): optional string fields now use `Option<&str>` and call sites pass `as_deref()`/string literals directly; `den-runtime`/`den-bearwire` clippy/tests green.
...
- [x] `src/memory/curation.rs:205-283` `sqlite_proposal_to_row` — ~15-step manual `.get().and_then().unwrap_or().to_string()` chain — replace with a `#[derive(Deserialize)]` struct. **DONE** (typed-boundary batch): added `SqliteProposalPayload` with serde defaults and simplified row conversion; `den-runtime` clippy green.
...
- [x] `src/memory/curation.rs:285-303` `sqlite_observation_to_row` fabricates `id: Uuid::new_v4()` and hardcodes `salience: "normal"` rather than reading from source — undocumented whether intentional. **DONE** (docs batch): added comment explaining SQLite observations predate the Postgres row-id/salience columns and the bridge supplies UI-only DTO metadata; `den-runtime` clippy/tests green.
...
- [x] `src/runtime/conversations.rs:106-120,155-169` `runtime_messages_top_array`/`runtime_conversations_top_array` — near-duplicate "unwrap array from one of several keys" logic — share a helper. **DONE** (duplication batch): added `top_array_from_keys` and reused it for messages/conversations; `den-runtime` clippy green.
- [x] `src/runtime/conversations.rs:171-181` `truncate_runtime_message` duplicates `summarize.rs:207-219`'s `truncate_chars` almost exactly — consolidate into one shared utility (3rd copy of this pattern in the crate, see also `key_memory_projection.rs`). **DONE** (duplication batch): added `runtime::text::truncate_chars` and reused it in conversations and compaction summarization; `den-runtime` clippy/tests green.
...
- [x] `src/runtime/compaction/mod.rs:240-244` `push_unique` duplicated verbatim in `summarize.rs:222-228` (3rd copy across the crate) — needs one shared helper. **DONE** (duplication batch): added `runtime::text::push_unique` and reused it in both compaction modules; `den-runtime` clippy/tests green.
...
- [x] `src/runtime/compaction_store.rs:130-169,208-244` — manual `row.try_get(...)` field-by-field decode (~9 fields, done twice) instead of `sqlx::FromRow`/`query_as` used elsewhere in the same module family. **DONE** (decode batch): added `CompactionStatusRow`/`CompactionArtifactRow` with `sqlx::FromRow` and converted manual decode blocks to `query_as`; `den-runtime` clippy/tests green.
- [x] `src/runtime/bearwire_projection/mod.rs:30-175` vs `wire.rs:168-346` — two large parallel matches transcode the same `RuntimeSemanticEvent` into two wire formats with duplicated error-category-to-string mapping — share one `RuntimeErrorCategory -> &str` function. **DONE** (duplication batch): added shared `runtime_error_category_code` and reused it in both projection paths; `den-runtime` clippy/tests green.
...
- [x] `src/agent_loop/session_store.rs:80,87,96,106` `.expect("agent loop session lock")` on every mutex access — will panic the whole process on lock poisoning; use `unwrap_or_else(|e| e.into_inner())` or a poison-tolerant wrapper. **RECONCILED**: current store methods use `unwrap_or_else(PoisonError::into_inner)` on all lock accesses; verified in `agent_loop/session_store.rs`.
...
- [x] `src/agent_assist/assistant_display.rs:70-83` `strip_prompt_scaffolding_prefix` hardcodes magic sentinel strings inline rather than named constants, inconsistent with `overflow_retry.rs:22`'s `COMPACTION_BLOCK_MARKER` const pattern. **DONE** (cleanup batch): introduced named prefix constants; `den-runtime` clippy green.
- [x] `src/agent_assist/agent_prefill.rs:54-58` — `while s.contains("--") { s = s.replace(...) }` O(n²)-ish manual collapse loop instead of a regex (already a dependency elsewhere in the crate). **DONE** (cleanup batch): uses a shared `Regex` via `LazyLock` to collapse repeated dashes; `den-runtime` clippy green.
- [x] `src/agent_loop/session_store.rs:66` `AgentLoopSessionStore` derives `Default` but also hand-writes a redundant `new()`. **DONE** (cleanup batch): removed redundant `new()` and updated tests to use `AgentLoopSessionStore::default()`; `den-runtime` clippy green.
...
- [x] `src/runtime/compaction/render.rs:6-42` — six repetitive `if !summary.X.is_empty() { sections.push(...) }` blocks — data-driven loop would cut ~35 lines. **DONE** (cleanup batch): section rendering now iterates over heading/item pairs; `den-runtime` clippy green.
- [x] `src/runtime/compaction/render.rs:44-49` — `sections.len() == 1` used as a fragile implicit "nothing rendered" check. **DONE** (cleanup batch): uses explicit `rendered_summary_fields` flag; `den-runtime` clippy green.
- [x] `src/runtime/compaction_observability.rs:29-65` `build_compaction_applied_event`/`build_compaction_skipped_event` hand-assemble the same 8-field struct — unify via `RuntimeCompactionEvent::skipped(...)`/`::applied(...)` associated functions. **DONE** (cleanup batch): added associated constructors and kept the existing free functions as compatibility wrappers; `den-runtime` clippy green.
...
- [x] `src/runtime/role_registry.rs:41-44` — synthetic binding-id convention (`format!("den-native:{bear_id}:...")`) isn't documented as part of `resolve_binding`'s public contract. **DONE** (docs batch): added comment documenting the deterministic fallback binding contract for native-only/pre-provisioned runs; `den-runtime` clippy green.
...
- [x] `src/recall/registry.rs:38,68,90,139,165,194` — six near-identical sqlx-error-wrapping lines — extract a shared `wrap(op)` helper. **DONE** (error cleanup): added shared `db_err(operation)` for recall registry SQL/decode errors; `den-service` clippy green.
- [x] `src/recall/registry.rs:101` `#[allow(clippy::too_many_arguments)]` on `upsert_passage` (9 params) — use a `NewPassage<'a>` struct (matching the `CreateBearObservation` convention already used in `bear_observations.rs`). **DONE** (typed-boundary batch): added `NewPassage<'a>` and updated indexer call site; `den-service` clippy green.
...
- [x] `src/memory/bear_observations.rs:44-51,77-79,96-98` — same 12-column SELECT/RETURNING list repeated verbatim 3 times — extract a `const OBSERVATION_COLUMNS`. **DONE** (duplication batch): added `OBSERVATION_COLUMNS` and reused it in create/update/get query shapes; `den-runtime` clippy green.
- [x] `src/native_runtime/tools.rs:27` `den_tools_for_profile(_config, role)` — unused `_config` parameter. **DONE** (cleanup batch): removed the unused leaf parameter; boundary `merge_den_and_client_tools` keeps `_config` for current API shape; `den-runtime` clippy green.
- [x] `src/native_runtime/tools.rs:57-59` `&first_sentence[..96]` byte-slices a `&str` from arbitrary tool descriptions — will panic on non-ASCII char boundary. **DONE** (previous safety batch): truncation now backs off to a UTF-8 char boundary before slicing; verified still present as fixed in code.
...
- [x] `src/native_runtime/profile_briefing.rs:9-26` — builds prompt via repeated `push_str(&format!(...))`, allocating a new String per item — use `write!(prompt, ...)`. **DONE** (cleanup batch): uses `writeln!` against the existing prompt buffer; `den-runtime` clippy green.
...
- [x] `src/bears/prompt_fragments/render.rs:45-46,60-61` `render_turn_text`/`render_compile_time_text` construct a brand-new `minijinja::Environment` per call — cache/share if this is a hot path. **DONE** (perf cleanup): added shared `LazyLock` strict MiniJinja environment used by both render paths; `den-service` clippy green.
...
- [x] `src/model_selection.rs:226-239` `resolve_model_option` — silently falls through to the static registry on any DB error (not just "not found"), masking real failures as fallback-worthy. **DONE** (correctness batch): DB errors now propagate via `?`; static registry fallback is used only when no DB row exists; `den-service` clippy green.
- [x] `src/model_selection.rs:80-90` `simplify_model_option_label_for_acp` — dense string-processing chain with no comment explaining expected label formats. **DONE** (docs batch): added comment describing metadata/token-limit suffixes stripped for ACP display; `den-service` clippy green.
