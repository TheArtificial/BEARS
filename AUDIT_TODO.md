...
- [ ] `src/oauth/utils.rs:20-32` vs `jwt.rs:239-251` — `generate_secure_random_string` and `generate_jti` duplicate the same charset-sampling loop.
...
- [ ] `agent_loop/key_memory_projection.rs:185-469` `project_key_memory` — ~285-line god function, 5 near-identical tier blocks — strongest extract-helper candidate in the crate.
...
- [ ] `src/reflection_outcomes.rs:70` `reflection_outcome_exists` swallows DB error via `.unwrap_or(0)`, treating real failures as "false".
...
- [ ] `src/reflection/conductor.rs:734-738` `native_curate_llm_briefing_enabled` reads env var on every call rather than caching via `OnceLock`/`LazyLock` (pattern used elsewhere, e.g. `turn.rs:54`).
...
- [ ] `src/reflection/conductor.rs:1336-1354` `row_from_sql` uses `row.get(...)` (panics on missing/wrong-typed column) instead of `try_get`, inconsistent with rest of file's `Result` style.
...
- [ ] `src/agent_assist/agent_summary.rs:23-46` vs `agent_prefill.rs:15-38` — identical `pick_str`/`model_field` helpers duplicated verbatim between files (3rd copy also in `agent_diagnostics.rs:53-64`) — share via an `agent_assist` util module.
...
- [ ] `src/turn_runs.rs:143,169` vs `turn_steps.rs:79,153` — active-state SQL lists (`'streaming_model', 'waiting_for_client', ...`) hand-duplicated as raw string literals in multiple places instead of derived from the state enum's variants — drift risk.
- [ ] `src/turn_steps.rs:110-114,137-141` `transition_step`/`transition_active_steps_for_run` take `state: &str` and re-validate via `try_from_storage`, inconsistent with `turn_runs.rs`'s `transition_run` which takes a typed enum directly.
...
- [ ] `src/bears/sync.rs:24,31,38` — sync status compared via literal `&str` (`"failed"`/`"skipped_missing_binding"`/`"synced"`) in 3 methods — use a `SyncStatus` enum with `Display`/`Serialize`.
- [ ] `src/bears/sync.rs:21,28` `failed_profiles`/`skipped_profiles` allocate a `Vec` per call even when immediately consumed via `.into_iter()` at the only call site — could return `impl Iterator`.
