...
- **Budgets are ledger-first.** Wall-clock, tool-class, failure, and ko budgets govern loop health; the model receives explicit low-budget warnings in-band, while hidden operational outcome records preserve the truth for future replay ([ADR-0050](docs/decisions/adr-0050-adaptive-turn-budgets-and-loop-ko.md)). Context-window pressure is tracked against the fully assembled request before inference ([ADR-0047](docs/decisions/adr-0047-context-window-budget-and-token-estimation.md)); on provider overflow, Den compacts and retries once, preserving in-flight tool results and unresolved approvals.
...
Budgets and loop health: [ADR-0050](docs/decisions/adr-0050-adaptive-turn-budgets-and-loop-ko.md) · [ADR-0047](docs/decisions/adr-0047-context-window-budget-and-token-estimation.md)
