...
Cabinet is the Den-wide shared knowledge wiki that humans and Bears read and edit together ([contract](docs/architecture/cabinet-contract.md)). The model sees: `cabinet_search`, `cabinet_read`, and `cabinet_history` on every stance; `cabinet_create`, `cabinet_update`, and `cabinet_source_link` on `chat`/`pair`/`curate`; and `cabinet_lifecycle` (archive/restore) on `curate` only. The distinction the descriptors teach:
