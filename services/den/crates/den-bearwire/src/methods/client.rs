...
const MODEL_TOOL_RESULT_MAX_CHARS: usize = 24 * 1024;
const TOOL_RESULT_FIELD_MAX_CHARS: usize = 12 * 1024;

#[derive(Debug, Clone)]
struct CompactToolResult {
    payload: Value,
    content: String,
}

fn truncate_str(value: &str, max_chars: usize) -> (String, bool, usize) {
...
fn compact_value_for_model(value: Value, max_chars: usize) -> (Value, bool, usize) {
    match value {
        Value::String(text) => {
...
fn compact_client_tool_result_params(
    tool_call_id: &str,
    status: &str,
    params: &Value,
) -> CompactToolResult {
    ...
    if model_content.chars().count() > MODEL_TOOL_RESULT_MAX_CHARS {
        let (text, _, _) = truncate_str(&model_content, MODEL_TOOL_RESULT_MAX_CHARS);
        model_content = text;
    }
    if truncated && !model_content.contains("result truncated") {
