...
//!
//! 1. **Native profile reconcile:** Trigger a bear reconcile (any code path that calls [`super::provision::reconcile_bear_native`])
...
//! 2. **MemFS Manager (git HTTP):** Root compose points Letta at `http://bears-memfs-manager:8285` and `LETTA_MEMFS_LOCAL=1` on
//!    codepool. Canonical on-disk memory lives on **Letta’s volume** (`bears-letta-data`,
